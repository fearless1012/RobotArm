    ...
    while True:
        if __isRunning:        
            if detect_color != 'None' and start_pick_up:  #If it is detected that the square has not moved for a period of time, start to grip
                #Move to the target position, the height is 6cm, and judge whether it can reach the specified position by the returned result
                #If the running time parameter is not given, it will be automatically calculated and returned by the result
                ...
                if result == False:
                    ...
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(1, servo1 - 200, 500)  # Open paws, put down objects
                    time.sleep(0.8)
